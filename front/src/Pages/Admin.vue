<template>
  ...
  >
    <div class="containerr flex justify-between">
      ...
      <div class="w-[80%] ml-5 p-8 bg-[var(--sc)] rounded-[20px] min-h-[83vh]">
        <div v-if="active === 'one'" class="flex flex-wrap gap-5">
          ...
          <img
            @click="addBook"
            class="w-[250px] cursor-pointer !border !border-[var(--yellow)]"
            src="../assets/add.png"
            alt="book cover"
          />
        </div>
        ...
        >
          ...
          >
            ...
            />
            <h2 class="text-xl font-bold mt-3 text-[22px]">
              {{ selectedBook.title }}
            </h2>
            ...
            <button
              @click="deleteee(selectedBook.id)"
              class="font-bold p-3 rounded-[10px] bg-[var(--yellow)] !mt-2 font-bold cursor-pointer"
            >
              Delete
            </button>
          </div>
        ...
        <div
          v-if="showPopup2 == true"
          class="fixed inset-0 rounded-[20px] backdrop-blur-sm bg-black/30 bg-opacity-50 flex justify-center items-center"
        >
          <div
            class="bg-white p-10 rounded-[20px] w-[400px] relative flex flex-col gap-2"
          >
            <button
            ...
            <p class="font-bold !mt-8">Cover Image :</p>
            <input
              @change="handleFile"
              type="file"
              accept="image/*"
            ...
            <button
              @click="add"
              class="w-[150px] font-bold bg-[var(--bg-nav)] text-[var(--white)] text-center rounded-[8px] cursor-pointer pt-3 pb-3 hover:bg-[var(--bg-nav-hover)] duration-[.4s] hover:scale-[1.03]"
            >
              add book
            </button>
          </div>
        </div>
        <div v-if="active === 'two'" class="">
        ...
        <div v-if="active === 'three'" class="mt-3 flex flex-col gap-8">
          <div v-for="user in usersData" :key="user.id" class="">
            <div
              class="flex border border-[var(--yellow)] p-3 rounded-[10px] items-center"
            >
              <img
                class="w-[150px] h-[150px] min-w-[150px] min-h-[150px] rounded-[50%]"
                :src="`http://localhost:8000/storage/${user.profile_image}`"
                alt="No Profile Image "
              />
              <div class="flex flex-col pl-10 gap-2">
                <p class="font-bold">id : {{ user.id }}</p>
                <p class="font-bold">email : {{ user.email }}</p>
                <p class="font-bold">name : {{ user.name }}</p>
                <p class="font-bold">role : {{ user.role }}</p>
                <button
    ...
    <div class="activee activedelete">Deleted successfully ✔</div>
    <div class="activee activeadd">Added successfully ✔</div>
    <div class="activee activereturn">Book Returned successfully ✔</div>
    <div class="activee activedeletuser">Student Deleted successfully ✔</div>
  </div>
...
export default {
  ...
  methods: {
    ...
    add() {
      ...
      if (this.changeimg) formData.append("cover_image", this.changeimg);
      if (this.auther) formData.append("author", this.auther);
      ...
      axios
        ...
        .then((res) => {
          ...
          const aa = document.getElementsByClassName("activeadd")[0];
    ...
    deleteUser(id) {
      axios
        ...
        .then((res) => {
          console.log(res);
          this.getUsersData();
          const aa = document.getElementsByClassName("activedeletuser")[0];
          aa.classList.add("block");
    ...
    getUserData() {
      axios
        ...
        .then((res) => {
          this.userData = res.data.data[this.id - 1];
    ...
    DeleteBook(id) {
      axios
        ...
        .then((res) => {
          ...
          const aa = document.getElementsByClassName("activereturn")[0];
          aa.classList.add("block");
